...
semd_PTR temp = helpTraverse(semAdd);
	semd_PTR new = temp -> s_next;
	
	/* if location is not active then allocate it - condition coming back as true means semdFree list is empty - false means it is not empty */
	if (temp->s_next->s_semAdd != semAdd) {
		new = allocSemd(temp, semAdd); /*gives ptr to prnt and address*/
	}		
	
	/* if comes back as true, the free list was empty */
	if (new == NULL){
		return TRUE;
	}
	...
	/*semaphore does not need to be allocated, list not empty, return false*/
	return FALSE;
...
semd_PTR blockRemove = helpTraverse(semAdd);
	pcb_PTR semHead;
	semd_PTR temp;
	
	if (blockRemove->s_next->s_semAdd != semAdd) {
		return NULL; /* desciptor not found in ASL */
	}
	
	/* removes head from queue and returns pointer */
	semHead = removeProcQ(&blockRemove -> s_next -> s_procQ); 
	
	/* removes descriptor from ASL & returns to free list */
	if(emptyProcQ(blockRemove -> s_next -> s_procQ)){
		temp = blockRemove -> s_next;
		blockRemove -> s_next = blockRemove -> s_next -> s_next;
		temp -> s_next = semdFree_h;
		semdFree_h = temp;
	}
	semHead -> p_semAdd = NULL;
	return semHead;
...
semd_PTR temp = helpTraverse(p -> p_semAdd);
	
	/*if sem is not active return null*/
	if (temp->s_next->s_semAdd != p->p_semAdd) {
		return NULL;
	}
	
	/*else its active - p needs to be returned*/
	p = outProcQ (&(temp -> s_next -> s_procQ), p);	
	if (emptyProcQ(temp -> s_next -> s_procQ)) {
		semd_PTR empty = temp -> s_next;
